package com.fm.designstar.widget.imagePicker.loader;
...
public class MediaHandler {
    ...
    public static List<MediaFolder> getMediaFolder(Context context, ArrayList<MediaFile> imageFileList, ArrayList<MediaFile> videoFileList) {
        ...
        if (!mediaFileList.isEmpty()) {
            MediaFolder allMediaFolder = new MediaFolder(ALL_MEDIA_FOLDER, context.getString(R.string.quan), mediaFileList.get(0).getPath(), mediaFileList);
            mediaFolderMap.put(ALL_MEDIA_FOLDER, allMediaFolder);
        }

        //全部视频
        if (videoFileList != null && !videoFileList.isEmpty()) {
            MediaFolder allVideoFolder = new MediaFolder(ALL_VIDEO_FOLDER, context.getString(R.string.quan), videoFileList.get(0).getPath(), videoFileList);
