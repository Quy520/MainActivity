...
public class MyViewGroup<X extends View> extends ViewGroup {
    ...
    public MyViewGroup(Context context, AttributeSet attrs) {
        super(context, attrs);
        mContext = context;
        HorInterval = Tool.dip2px(mContext, 15);
    ...
    public void setAdapter(BaseGroupAdapter adapter) {
        removeAllViews();
        if (adapter.getCount() != 0) {
            for (int i = 0; i < adapter.getCount(); i++) {
                addItemView(adapter.getView(this, i));
            }
        }
    }
